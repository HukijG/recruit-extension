...
import { TEMPLATES_STORAGE_KEY } from "~/lib/constants"
import { useStorage } from "~/lib/storage"
import { substituteFirstName } from "~/lib/templates"
import type { SmsTemplate } from "~/lib/types"

// SMS composer ported from the extension. sendToBackground swap is the
// only logic edit; all popover behaviour (autofocus, confirm-yes/no,
// retry-after countdown plumbing) stays as-is.
...
export function TextPopover({
  ...
  const [stored] = useStorage<SmsTemplate[]>(TEMPLATES_STORAGE_KEY, [])
  ...
  const handleYes = async () => {
    ...
    const resp = await sendDialpadSms({
      phoneNumber,
      callerAliasId,
      text,
  ...
  return (
    <div
      ...
      {managerOpen && <TemplateManager onClose={() => setManagerOpen(false)} />}
    </div>
  )
}
