// SMS template storage. Local-only (browser localStorage via the localStore
// shim in constants.ts), so templates live per-device for now. Same shape
// as the extension so future remote-sync wraps the same helpers.
